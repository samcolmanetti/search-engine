...
- [x] 1.1 Rename `UrlUtilTester`, `RobotsHandlerTester`, `ScaledStemmerTester`, and
      `DocumentProcessorTester` in `SE` to `*Test`. Check that `mvn test` reports a nonzero test count.
- [x] 1.2 `ScaledStemmerTest`: drop the Spring runner and `TestConfig`, and use `new ScaledStemmer()`.
- [x] 1.3 `RobotsHandlerTest`: stop using the network. `RobotsHandler.add` opens a URL stream, so
...
- [x] 1.4 `DocumentProcessorTest`: build documents with `Jsoup.parse(html, baseUri)` from
      fixtures and use real assertions.
- [x] 1.5 Delete `TestConfig` once no test uses it.
- [x] 1.6 `SR`: remove `SearcherApplicationTests.contextLoads` (reason in the proposal) and
      replace it with the unit tests in section 3.

## 2. New unit tests in `server/search-engine`
- [x] 2.1 `LRUCacheMap`: evicts the least recently *accessed* entry, not just the oldest inserted,
      and respects the default limit of 128.
- [x] 2.2 `LRUCacheSet`: `add` returns true for new items and false for duplicates. It also
      covers `contains`, eviction, `size`, and `isEmpty`.
- [x] 2.3 `NodeIterable`: pre-order depth-first traversal order, and `remove` throws
      `UnsupportedOperationException`.
- [x] 2.4 `StopWordsUtil`: common stop words are detected, real words are not, and the check is
      case-sensitive (callers lowercase first).
- [x] 2.5 `CollectionsUtil.emptyIfNull`.
- [x] 2.6 `JdbcUtil.getInsertedId` with Spring's `GeneratedKeyHolder`: a single key, multiple keys
      that include the id column, and multiple keys without it (returns -1).
- [x] 2.7 `UrlUtil`: more cases for `getCleanUrl` (strips fragment and query, forces http,
      adds a missing scheme, encodes spaces), `getUrlKey`, `getUrlString`, `getRobotsTxtURL`,
      and invalid input returning null.
- [x] 2.8 `TermProcessor`: counts words, ignores stop words and 1-character tokens, strips
      punctuation, lowercases, and merges words that share a stem under one key. Use `Jsoup.parse`
      input and `getTermInfos()`.
- [x] 2.9 `InMemoryIndexer`: `indexPage`, `get`, `getCounts`, and `shouldIndex` (true for a new
      URL, false right after indexing, false for an invalid URL). A null document is a no-op.
- [x] 2.10 `DatabaseIndexer` with fake `DocumentInfoDao` and `TermInfoDao` implementations:
      upserts the document, then its terms, and skips the terms when the returned id is 0 or less.
- [x] 2.11 `Crawler` with fakes: a test subclass of `Fetcher` that overrides
...
- [x] 2.12 Mappers (`DocumentInfoMapper`, `TermInfoMapper`): only if a `ResultSet` stub is cheap
      (a `java.lang.reflect.Proxy` works without Mockito). Otherwise skip.

## 3. New unit tests in `server/searcher/seacher`
- [x] 3.1 `TermFrequencyRanker`: an empty ranker returns an empty list; relevance is computed as
...
- [x] 3.2 `SearcherImpl` with a fake `SearchInfoDao`: searches for the stemmed term first, falls
...
- [x] 3.3 `SearchController`: an empty query returns `{}`. Set the private `searcher` field via
      `ReflectionTestUtils`, or wait for the constructor-injection task in `tidy-code-quality`.
- [x] 3.4 `SearchInfo` and `SearchResult`: skip plain getters and setters.

## 4. Coverage
- [x] 4.1 Add `jacoco-maven-plugin` (latest release) to both poms with `prepare-agent` and
      `report` bound to `verify`. Don't set a threshold.
- [x] 4.2 Confirm `mvn verify` passes in both projects. Note the coverage numbers in the PR body.
